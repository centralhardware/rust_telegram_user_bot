...
pub fn rich_text(message: &Message) -> Option<String> {
    rich_text_of(&message.raw)
}

/// [`rich_text`], for the TL message on its own.
pub fn rich_text_of(message: &tl::enums::Message) -> Option<String> {
    render(extract_rich(message)?)
}
...
fn render_text(text: &tl::enums::RichText) -> String {
    ...
    match text {
        ...
        T::TextUnderline(t) => crate::render::format_entities::underline(&render_text(&t.text)),
        T::TextStrike(t) => crate::render::format_entities::strike(&render_text(&t.text)),
